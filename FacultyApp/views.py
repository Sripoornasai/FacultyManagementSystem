...
def DownloadMarks(request):
    if request.method == 'GET':
        ...
        context= {'data':output , 'image':'../static/images/Faculty-4.jpg'}
...
def ViewEngagement(request):
    if request.method == 'GET':
        ...
        output+= "</table></br></br></br></br>"        
        context= {'data':output , 'image':'../static/images/images_1.jpg'}
        return render(request, 'StudentScreen.html', context)
        ...
        with con:    
            ...
            for row in rows:
                if row[0] == username and password == row[1]:
                    ...
                    break		
        if index == 1:
            context= {'data':'welcome '+username , 'image':'../static/images/Faculty-4.jpg'}
            return render(request, 'StudentScreen.html', context)
        else:
            context= {'data':'login failed'}
